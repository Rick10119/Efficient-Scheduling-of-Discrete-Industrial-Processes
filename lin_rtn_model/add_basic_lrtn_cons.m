...
cons_basic_lrtn = [cons_basic_lrtn, R_IT(:, 2 : end) <= R_IT(:, 1 : end - 1) + temp - temp1];% change of resource

% resource upper limit
cons_basic_lrtn = [cons_basic_lrtn, R_IT(:, 2 : end) >= 0];
cons_basic_lrtn = [cons_basic_lrtn, R_IT(1 : index_resource_mat_s(end), 2 : end) <= 1];

% initial resources 
cons_basic_lrtn = [cons_basic_lrtn, R_IT(:, 1) == 0];


%% Task execution (b)
% the total operating time in a time slot:
cons_basic_lrtn = [cons_basic_lrtn, sum(D_IKT, 2) == 1];% sum the second dimension

% non-negative & less than 1
cons_basic_lrtn = [cons_basic_lrtn, D_IKT <= 1];
cons_basic_lrtn = [cons_basic_lrtn, D_IKT >= 0];
...
cons_basic_lrtn = [cons_basic_lrtn, ...
...
cons_basic_lrtn = [cons_basic_lrtn, R_IT(end, end) == NOF_HEAT];
